...
function readState() {
  if (!fs.existsSync(STATE_FILE)) return { targets: {} };
  try {
...
function normalizeTarget(item, index) {
  ...
  const defaultWaitUntil = env("DEFAULT_WAIT_UNTIL", "networkidle");
  ...
  return {
    ...
    userAgent: item.userAgent || "Mozilla/5.0 GitHub-Actions-RealPage-Keepalive/1.0",

    screenshot: item.screenshot !== false,
    fullPageScreenshot: item.fullPageScreenshot === true,
    ...
    expectedSelector: item.expectedSelector || null,
...
async function loginIfNeeded(page, target) {
  ...
  if (login.usernameSelector && login.usernameEnv) {
    const username = env(login.usernameEnv);
    if (!username) throw new Error(`登录用户名环境变量为空：${login.usernameEnv}`);
    await page.fill(login.usernameSelector, username, { timeout: target.timeoutMs });
  }

  if (login.passwordSelector && login.passwordEnv) {
    const password = env(login.passwordEnv);
    if (!password) throw new Error(`登录密码环境变量为空：${login.passwordEnv}`);
    await page.fill(login.passwordSelector, password, { timeout: target.timeoutMs });
  }

  if (login.submitSelector) {
    await Promise.all([
      page.waitForLoadState("domcontentloaded", { timeout: target.timeoutMs }).catch(() => {}),
      page.click(login.submitSelector, { timeout: target.timeoutMs })
    ]);
  }
  ...
  for (let attempt = 1; attempt <= target.maxRetries; attempt++) {
    ...
    try {
      ...
      if (target.screenshot) {
        const safeName = target.name.replace(/[^\w.-]+/g, "_").slice(0, 80);
        screenshotPath = path.join(SCREENSHOT_DIR, `${safeName}_${id}.png`);
      ...
      try {
        ensureDir(SCREENSHOT_DIR);
        const safeName = target.name.replace(/[^\w.-]+/g, "_").slice(0, 80);
        screenshotPath = path.join(SCREENSHOT_DIR, `${safeName}_${id}_error.png`);
...
function buildReport(results, changes) {
  ...
  return {
    ...
    summary: {
      ...
      failed,
      avgLatencyMs,
      notifyMode: env("NOTIFY_MODE", "change"),
...
function markdownTable(results) {
  ...
  for (const r of results) {
    lines.push(
      `| ${r.ok ? "✅ OK" : "❌ FAIL"} | ${r.name} | ${r.status ?? "-"} | ${r.latencyMs} ms | ${r.attempts} | ${String(r.reason).replace(/\|/g, "\\|")} | ${r.screenshotPath || "-"} |`
    );
  }
...
function shouldNotify(report) {
  const mode = env("NOTIFY_MODE", "change").toLowerCase();
